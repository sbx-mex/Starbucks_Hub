...
function homeToolRecords() {
  return toolRecords().filter((record) => normalizeBool(record.Inicio));
}
...
function renderHome() {
  ...
  $("#home-tools-grid").innerHTML = records.map((record, index) => {
    const name = String(record.Nombre || "Herramienta").trim();
    const title = esc(name);
    const iconText = esc(record.Icono || "•");
    const description = esc(record.Notas || HOME_TOOL_DESCRIPTIONS[name] || "Acceso operativo del Hub.");
    if (!externalUrl(record.URL)) {
      return `<article class="home-tool-card is-disabled" aria-disabled="true">
        ...
        <span class="home-tool-state">${esc(toolStatusLabel(record))}</span>
      </article>`;
    }
...
function renderLinks() {
  ...
  const favorites = records.filter((record) => normalizeBool(record.Inicio));
  ...
  const filtered = rankedRecords.filter((record) => {
    ...
    const matchesFilter = state.toolFilter === "all"
      || (state.toolFilter === "favorites" && normalizeBool(record.Inicio))
