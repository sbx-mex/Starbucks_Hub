...
tools = cms["sheets"]["Herramientas"]
project_tools = [
    record for record in tools
    if str(record.get("Nombre") or "").strip() and not str(record.get("URL") or "").strip()
]
invalid_home_values = [
    record.get("Nombre") for record in tools
    if sort_text(record.get("Inicio")) not in {"si", "no"}
]
if invalid_home_values:
    fail("Inicio solo acepta Si o No: " + ", ".join(map(str, invalid_home_values)))
home_tools = [record for record in tools if sort_text(record.get("Inicio")) == "si"]
...
if "HOME_TOOL_NAMES" in js:
    fail("Inicio aún contiene una lista fija fuera del CMS")
if "function homeToolRecords()" not in js or "normalizeBool(record.Inicio)" not in js:
    fail("Inicio no está controlado por la columna Inicio del CMS")
if "Selecciona Inicio = Si" not in js:
    fail("Inicio no explica cómo resolver una selección vacía desde el CMS")
...
print(f"Herramientas en Inicio: {len(home_tools)} · controladas por Inicio = Si/No")
print(f"Herramientas en proyecto: {len(project_tools)} · pueden ser cero y se mantienen visibles sin URL")
